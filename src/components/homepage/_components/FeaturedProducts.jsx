...
const ProductCard = React.memo(
  ({ item, handleCart, handleWishlist, isInWishlist, isInCart }) => {
    ...
    ${storedRole === "admin" ? "hidden" : ""}`}
          ...
          {hasDiscount && (
            <div className="absolute top-3 left-3 bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-md">
              -{discount}{" "}
              {item?.promotion_discount_type === "percentage" ? "%" : "XAF"}
            </div>
          )}
          ...
          <div className="p-5">
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col">
                ...
                <span className="text-[#CBA135] popbold text-[16px]">
                  XAF {newPrice}
                </span>
              </div>
              <button
...
const FeaturedProducts = ({allProducts, isLoading, isError, title, subtitle}) => {
  ...
  const handlePageChange = useCallback((page) => {
    ...
    if (productsGridRef.current) {
      ...
      const y =
        productsGridRef.current.getBoundingClientRect().top +
        window.pageYOffset +
        yOffset;
  ...
  if (isError)
    return (
      <p className="p-20 text-center text-red-500">Failed to load products</p>
    );
    ...
    >
      ...
      >
        {currentProducts.length === 0 ? (
          <div className="col-span-4 text-center py-10">No product found</div>
        ) : (
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-10 gap-2 flex-wrap">
          ...
          <button
            ...
            className={` ${
              currentPage === 1 ? "hover:text-red-500 hover:cursor-pointer" : ""
            }`}
          >
            <ArrowLeft />
          </button>
