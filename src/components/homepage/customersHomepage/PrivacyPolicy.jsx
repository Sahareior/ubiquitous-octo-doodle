...
const PrivacyPolicy = () => {
  const { data: updatedPrivacy, isLoading, isError } = useGetPrivacyCustomersQuery();
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#FAF8F2]">
        <div className="text-lg font-semibold text-gray-600">Loading Privacy Policy...</div>
      </div>
    );
  }

  if (isError || !updatedPrivacy) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#FAF8F2]">
        <div className="text-lg font-semibold text-red-600">Failed to load Privacy Policy.</div>
      </div>
    );
  }

  const { content, updated_at, title } = updatedPrivacy;
